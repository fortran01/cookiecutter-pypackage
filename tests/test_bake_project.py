...
def test_make_help(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        if sys.platform != "win32":
            ...
            assert b"test-all is meant to test everything" in output
...
def test_bake_with_argparse_console_script_cli(cookies):
    ...
    result = cookies.bake(extra_context=context)
    project_path, project_slug, project_dir = project_info(result)
    cli_module_path = os.path.join(project_dir, "cli.py")
    module_name = "{}.cli".format(project_slug.replace("-", "_"))
    spec = importlib.util.spec_from_file_location(module_name, cli_module_path)
    cli_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(cli_module)
    # Now directly call the main function
    assert cli_module.main() == 0  # Assuming the CLI function returns

# Moved dev requirements to setup.py
